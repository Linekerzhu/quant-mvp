...
### FATAL-1 [P1]: CPCV Date-Based Segment Splitting
...
| **级别** | P1 |
| **问题** | 按行号切分导致横截面泄漏 |
| **当前影响** | 20 sym下≈0%，500 sym下0.5%+ |

**修复**: 按df['date'].unique()切segment，再映射回indices。

---

### FATAL-2 [P1]: FracDiff d_opt移入CPCV Fold

| 属性 | 值 |
|------|-----|
| **来源** | 外部专家 + R19-F4 |
| **级别** | P1 |
| **问题** | d_opt全局优化有前视偏差，d差异0.33 |

**修复**: d_opt作为fold-internal参数，每fold重新计算。
...
| **修复日期** | 2026-03-02 |
| **Commit** | `2d5dda3` |
| **涉及文件** | `triple_barrier.py`, `meta_trainer.py`, `purged_kfold.py` |
| **修复类型** | 架构修正 |
| **严重程度** | P0-P3 |
...
| 修复 | 级别 | 描述 |
|------|------|------|
| FIX-2 | P1 | Triple Barrier引入side参数，做空barrier翻转 |
| FIX-3 | P2 | FracDiff移至side过滤之前计算 |
| FIX-4 | P2 | Inner-train/validation之间添加purge buffer |
| FIX-5 | P2 | Purge缺失列强制报错 |
| FIX-7 | P3 | val_size上下限保护 |
| FIX-8 | P3 | LightGBM固定seed |
| FIX-9 | P3 | _combinations改用math.comb |
...
|--------|------|------|
| 测试通过 | ✅ 165/165 | |
| Triple Barrier | ✅ | side×label逻辑兼容 |
| FracDiff | ✅ | 在全量数据上计算 |
| Purge Buffer | ✅ | 防止持有期重叠 |

**状态**: ✅ 已修复

---

## R24 - df_filtered变量引用修复 (P0)
...
R23-F1最终模型训练使用`df_filtered[features_with_fracdiff]`，但测试中Gate始终BLOCK导致此代码未执行。真实数据Gate PASS时会抛出KeyError（fracdiff列缺失）。

### 修复内容

- 3处 `df_filtered` → `df_meta`
- df_meta在L482-525已添加fracdiff列，始终可用
...
|--------|------|------|
| 测试通过 | ✅ 165/165 | |
| 变量引用 | ✅ | df_meta包含fracdiff |

**状态**: ✅ 已修复

---

## R23 - Model/Features Persistence
...
| **Commit** | `d5ed748` |
| **涉及文件** | `src/models/meta_trainer.py` |
| **修复类型** | Feature (Phase D readiness) |
| **严重程度** | P2→Phase D |

### 问题描述

MetaTrainer.train() 仅返回统计指标，未返回训练好的model对象。Phase D推理部署需要：
1. LightGBM model → model.predict(X_new)
2. 特征列表+顺序 → 确保推理时特征一致

### 修复内容

- Gate PASS后用全量数据训练最终模型
- 返回值添加: `model`, `feature_list`, `n_training_samples`
- 使用CPCV paths的中位数best_iteration作为最终模型轮数
...
| 编号 | 日期 | 描述 | 文件 | 状态 |
|------|------|------|------|------|
| FIX-2~9 | 2026-03-02 | 工程修正指南 (Triple Barrier side/FracDiff位置/Purge Buffer等) | multiple | ✅ 已修复 |
| R24 | 2026-03-02 | df_filtered→df_meta修复P0变量引用 | `meta_trainer.py` | ✅ 已修复 |
| R23 | 2026-03-02 | MetaTrainer.train()返回model对象 | `meta_trainer.py` | ✅ 已修复 |
